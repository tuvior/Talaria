...
class TestHBC59(unittest.TestCase):
    ...
    def test_get_function(self):
        # hbcdump version 59 cannot fetch
        ...
        for i in range(functionCount):
            ...
            target_functionName, _, target_paramCount, target_registerCount, target_symbolCount = (
                target_args[i]
            )
    ...
    def test_get_string(self):
        target_strings = re.findall(
            r"[isp][0-9]+\[([UTFASCI16-]+), ([0-9]+)..([0-9-]+)\].*?:\s?(.*)", self.pretty
        )
...
class TestParser59(unittest.TestCase):
    def test_hbc(self):
        ...
        hbc = hbcl.load(f)
        f.close()
        f = open("/tmp/talaria_test.android.bundle", "wb")
        hbcl.dump(hbc, f)
        f.close()
        ...
        a = f.read()
        f.close()
        f = open("/tmp/talaria_test.android.bundle", "rb")
        b = f.read()
        f.close()
    ...
    def test_tasm(self):
        ...
        a = hbcl.load(f)
        f.close()
        tasm.dump(a, "/tmp/talaria_test", force=True)
        b = tasm.load("/tmp/talaria_test")
